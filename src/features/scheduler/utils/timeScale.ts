...
export function getTimeScaleConfig(
  ...
  switch (preset) {
    case "hourAndDay": {
      ...
      if (isToday) {
        // Start 3 hours before current hour, show 24h window
        startDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours() - 3, 0, 0, 0);
        endDate = new Date(startDate.getTime() + 24 * 60 * 60 * 1000);
      } else {
        // Other days: midnight to midnight
